...
import sys
sys.path.insert(0, './')
from InGame.basketballShootOut.basketballShootOut import startGame
...
class GameStateController:
    ...
    def display_InstructionScreen(self):
        state = instructionScreen("basketball")
        if state == "back":
            self.display_PauseScreen()
            return
    ...
    def GetLocalHighScore(self):
        user_highscore = self.firebase_instance.GetLocalHighScore("BasketballShootOut")
        return user_highscore

    # Update(if possible) & return the local highscore of the user
    def UpdateLocalHighScore(self, score):
        user_highscore = self.firebase_instance.UpdateLocalHighScore("BasketballShootOut", score)
        return user_highscore

    # Update(if possible) & return the global highscore of the game
    def UpdateGlobalHighScore(self, score):
        user, score = self.firebase_instance.UpdateGlobalHighScore("BasketballShootOut",score)
        return user, score
